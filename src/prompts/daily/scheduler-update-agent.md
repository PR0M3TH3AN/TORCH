...
4. Correct agent names — The `Agent Name` column is derived from the
   filename by stripping the `` prefix and `.md` suffix.
...
- Self-exclusion awareness. This agent's own prompt file
  (`scheduler-update-agent.md`) lives in `daily/` and must appear in
...
2. Inventory — Daily prompts
   ...
   - Sort alphabetically.
   - For each file, derive the agent name: strip `` prefix and `.md`
     suffix (e.g., `audit-agent.md` becomes `audit-agent`).
