...
3. `src/lib/eventSchemas.js`, `src/lib/clientFacade.js`, `src/lib/defaultClient.js` — canonical builders & clients
4. `docs/event-schemas.md` — canonical event shapes
5. This agent prompt
...
- Using canonical builders / sanitizers from `src/lib/eventSchemas.js` (e.g., `buildVideoPostEvent`).
- Using `src/lib/clientFacade.js` or `src/lib/defaultClient.js` to talk to relays.
...
1) Preflight
   ...
   - Verify required files exist:
     - `src/lib/eventSchemas.js` (builders/sanitizers)
...
2) Implement harness
   ...
   - Core responsibilities:
     ...
     - Instantiate `sharedClient` / facade pointed at the configured relays.
     - For each event type:
       ...
       - Validate:
         ...
         - content/type/tags conform to `docs/event-schemas.md`.
     - VIEW_EVENT flow:
...
6) PR & commit
   ...
   - Commit message examples:
     - `test(ai): add interop harness for shared relay roundtrip`
